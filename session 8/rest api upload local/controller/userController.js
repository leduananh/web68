...
const { checkFileExists } = require("../helper/fileHelper")
...
const userController = (router, service) => {
    ...
    router.post('/', checkLoginMiddleware, require("../multerConfig").single("avatar"), asyncHandler(async (req, res, next) => {
        ...
        if (req.file && checkFileExists(req.file.path)) {
            userPayload.avatarUrl = `${req.protocol}://${req.get('host')}/${req.file.filename}`;
        }
